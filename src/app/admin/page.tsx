...
import { Building2, CheckCircle, XCircle, Clock, Eye, Ban, Copy, Check, Info, MapPin, Phone, Mail, Globe, Instagram as InstagramIcon, MoreVertical, Edit, Settings } from 'lucide-react'
    ...
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
              ...
              <p className="text-gray-500 mt-1 text-sm sm:text-base">Gerenciar empresas do Guia Marajoara</p>
            </div>
            <div className="flex gap-2 sm:gap-4 w-full sm:w-auto flex-wrap">
              <Link href="/admin/categorias" className="flex-1 sm:flex-none px-4 py-2 bg-orange-600 text-white rounded-lg hover:bg-orange-700 text-center text-sm">
                Categorias
              </Link>
              <Link href="/admin/planos" className="flex-1 sm:flex-none px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 text-center text-sm">
                Planos
              </Link>
              <Link href="/admin/destaques" className="flex-1 sm:flex-none px-4 py-2 bg-yellow-600 text-white rounded-lg hover:bg-yellow-700 text-center text-sm">
                Destaques
              </Link>
              <Link href="/admin/mari" className="flex-1 sm:flex-none px-4 py-2 bg-pink-600 text-white rounded-lg hover:bg-pink-700 text-center text-sm">
              Mari
              </Link>
              <Link href="/" className="flex-1 sm:flex-none px-4 py-2 border border-gray-300 rounded-lg hover:bg-gray-50 text-center text-sm">
                Ver Site
              </Link> 
              <button onClick={signOut} className="flex-1 sm:flex-none px-4 py-2 text-sm border border-gray-300 rounded-lg hover:bg-gray-50">
                Sair
              </button>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-8">
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6 mb-6 sm:mb-8">
          ...
          <div className="bg-white rounded-lg shadow p-4 sm:p-6">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-xs sm:text-sm font-medium text-gray-500">APROVADAS</h3>
          ...
          <div className="bg-white rounded-lg shadow p-4 sm:p-6">
            <div className="flex items-center justify-between mb-2">
              <h3 className="text-xs sm:text-sm font-medium text-gray-500">REJEITADAS</h3>
        ...
        <div className="bg-white rounded-lg shadow p-3 sm:p-4 mb-4 sm:mb-6 space-y-4">
          <div>
            <input
              type="text"
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex gap-2 overflow-x-auto scrollbar-hide">
              <span className="text-xs sm:text-sm text-gray-500 whitespace-nowrap self-center mr-2">Status:</span>
              <button onClick={() => setFilter('all')} className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium whitespace-nowrap ${filter === 'all' ? 'bg-[#C2227A] text-white' : 'bg-gray-100 text-gray-700'}`}>
                Todas
              </button>
              <button onClick={() => setFilter('pending')} className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium whitespace-nowrap ${filter === 'pending' ? 'bg-yellow-600 text-white' : 'bg-gray-100 text-gray-700'}`}>
                Pendentes
              </button>
              <button onClick={() => setFilter('approved')} className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium whitespace-nowrap ${filter === 'approved' ? 'bg-green-600 text-white' : 'bg-gray-100 text-gray-700'}`}>
                Aprovadas
              </button>
              <button onClick={() => setFilter('rejected')} className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium whitespace-nowrap ${filter === 'rejected' ? 'bg-red-600 text-white' : 'bg-gray-100 text-gray-700'}`}>
                Rejeitadas
              </button>
            </div>

            <div className="flex gap-2 overflow-x-auto scrollbar-hide">
              <span className="text-xs sm:text-sm text-gray-500 whitespace-nowrap self-center mr-2">Plano:</span>
              <button onClick={() => setPlanFilter('all')} className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium whitespace-nowrap ${planFilter === 'all' ? 'bg-[#C2227A] text-white' : 'bg-gray-100 text-gray-700'}`}>
                Todos
              </button>
              <button onClick={() => setPlanFilter('basic')} className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium whitespace-nowrap ${planFilter === 'basic' ? 'bg-gray-700 text-white' : 'bg-gray-100 text-gray-700'}`}>
                Básico
              </button>
              <button onClick={() => setPlanFilter('premium')} className={`px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium whitespace-nowrap ${planFilter === 'premium' ? 'bg-purple-600 text-white' : 'bg-gray-100 text-gray-700'}`}>
                Premium
              </button>
            </div>
          </div>
                ...
                {filteredBusinesses.map(business => (
                  <tr key={business.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4 text-sm text-gray-600">
                      {business.email_business && <div>{business.email_business}</div>}
            ...
            filteredBusinesses.map(business => (
              <div key={business.id} className="bg-white rounded-lg shadow p-4 relative">
                ...
                {business.email_business && (
                  <p className="text-sm text-gray-600 mb-1">{business.email_business}</p>
                )}
                <p className="text-xs text-gray-500">{new Date(business.created_at).toLocaleDateString('pt-BR')}</p>

                {showActionsMenu === business.id && (
                  <div className="absolute right-4 top-16 bg-white border rounded-lg shadow-lg z-10 py-1 min-w-[160px]">
      ...
      {showDetailsModal && selectedBusiness && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
          <div className="bg-white rounded-lg max-w-3xl w-full p-4 sm:p-6 my-8">
            <div className="flex justify-between items-start mb-4 sm:mb-6">
              <div className="flex-1 pr-2">
                <h2 className="text-xl sm:text-2xl font-bold">{selectedBusiness.name}</h2>
                <p className="text-sm sm:text-base text-gray-500">{selectedBusiness.category_sub || selectedBusiness.category_main}</p>
              </div>
            ...
            <div className="space-y-4 sm:space-y-6">
              {selectedBusiness.description && (
                <div>
                  ...
                  <p className="text-sm sm:text-base text-gray-600">{selectedBusiness.description}</p>
                </div>
              )}

              <div>
                <h3 className="font-semibold mb-2 text-sm sm:text-base">Informações de Contato</h3>
              ...
              <div>
                <h3 className="font-semibold mb-2 text-sm sm:text-base">Endereço</h3>
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-6 sm:mt-8">
              <button onClick={() => setShowDetailsModal(false)} className="flex-1 px-6 py-3 border border-gray-300 rounded-lg hover:bg-gray-50 text-sm sm:text-base">
                Fechar
              </button>
              <button onClick={() => editAsAdmin(selectedBusiness.id)} className="flex-1 px-6 py-3 bg-purple-600 text-white rounded-lg hover:bg-purple-700 text-sm sm:text-base">
                Editar como Admin
              </button>
              {selectedBusiness.status === 'pending' && (
                <>
                  <button onClick={() => rejectBusiness(selectedBusiness.id, selectedBusiness.name)} className="flex-1 px-6 py-3 bg-red-600 text-white rounded-lg hover:bg-red-700 text-sm sm:text-base">
                    Rejeitar
                  </button>
                  <button onClick={() => approveBusiness(selectedBusiness)} className="flex-1 px-6 py-3 bg-green-600 text-white rounded-lg hover:bg-green-700 text-sm sm:text-base">
                    Aprovar
                  </button>
                </>
              )}
      ...
      {showCredentialsModal && credentials && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-2xl w-full p-4 sm:p-6">
            <h2 className="text-xl sm:text-2xl font-bold mb-4 text-green-600">Empresa Aprovada!</h2>
            <p className="text-sm sm:text-base text-gray-600 mb-6">As credenciais de acesso foram criadas. Copie e envie para o parceiro:</p>

            <div className="space-y-4 bg-gray-50 p-3 sm:p-4 rounded-lg">
              <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">Email de acesso:</label>
                <div className="flex gap-2">
                  <input type="text" value={credentials.email} readOnly className="flex-1 px-3 sm:px-4 py-2 border rounded-lg bg-white text-sm" />
                  <button onClick={() => copyToClipboard(credentials.email, 'email')} className="px-3 sm:px-4 py-2 bg-[#C2227A] text-white rounded-lg hover:bg-[#A01860] flex items-center gap-2 text-sm whitespace-nowrap">
              ...
              <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">Senha temporária:</label>
                <div className="flex gap-2">
                  <input type="text" value={credentials.password} readOnly className="flex-1 px-3 sm:px-4 py-2 border rounded-lg bg-white font-mono text-sm" />
                  <button onClick={() => copyToClipboard(credentials.password, 'password')} className="px-3 sm:px-4 py-2 bg-[#C2227A] text-white rounded-lg hover:bg-[#A01860] flex items-center gap-2 text-sm whitespace-nowrap">
              ...
              <div>
                <label className="block text-xs sm:text-sm font-medium text-gray-700 mb-1">Link de acesso:</label>
                <div className="flex gap-2">
                  <input type="text" value={credentials.loginUrl} readOnly className="flex-1 px-3 sm:px-4 py-2 border rounded-lg bg-white text-sm" />
                  <button onClick={() => copyToClipboard(credentials.loginUrl, 'url')} className="px-3 sm:px-4 py-2 bg-[#C2227A] text-white rounded-lg hover:bg-[#A01860] flex items-center gap-2 text-sm whitespace-nowrap">
                    {copiedField === 'url' ? <Check size={18} /> : <Copy size={18} />}
            ...
            <div className="mt-6 bg-yellow-50 border border-yellow-200 rounded-lg p-3 sm:p-4">
              <p className="text-xs sm:text-sm text-yellow-800"><strong>Importante:</strong> Guarde essas credenciais. O parceiro deve fazer login e trocar a senha no primeiro acesso.</p>
            </div>

            <button onClick={() => setShowCredentialsModal(false)} className="mt-6 w-full px-6 py-3 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 font-semibold text-sm sm:text-base">
              Fechar
            </button>
          </div>
        </div>
      ...
      {showEditPlanModal && editPlanData && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-lg max-w-2xl w-full p-4 sm:p-6">
            <h2 className="text-xl sm:text-2xl font-bold mb-4">Editar Plano - {editPlanData.business.name}</h2>
            <p className="text-sm text-gray-600 mb-6">Ajuste as características do plano desta empresa específica</p>
