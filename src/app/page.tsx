...
import { useToast } from '@/components/ui/Toast' // Removido ToastContainer
...
export default function TestPage() {
  ...
  const handleSearch = (value: string) => { addToast({ title: 'Busca realizada', description: `Você buscou por: ${value}`, variant: 'info' }) }
  const showSuccessToast = () => { addToast({ title: 'Sucesso!', description: 'Operação realizada com sucesso.', variant: 'success' }) }
  const showErrorToast = () => { addToast({ title: 'Erro ao processar', description: 'Ocorreu um erro. Tente novamente.', variant: 'error' }) }
  const showWarningToast = () => { addToast({ title: 'Atenção', description: 'Verifique os dados informados.', variant: 'warning' }) }
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <section className="mb-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card variant="business"><CardHeader><CardTitle>Pizzaria Bella Italia</CardTitle></CardHeader></Card>
            <Card variant="coupon"><CardHeader><CardTitle>Desconto Especial</CardTitle></CardHeader></Card>
          </div>
        </section>
